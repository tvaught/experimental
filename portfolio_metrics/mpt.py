...
from metrics import (annualized_adjusted_rate, beta_bb, 
    expected_return, rate_array, stdp, volatility, TRADING_DAYS_PER_YEAR)


# Constants ####
price_schema = np.dtype({'names':['symbol', 'date', 'open', 'high', 'low', 'close', 'volume', 'adj_close'], 'formats':['S8', 'M8', float, float, float, float, float, float]})
